...
export const patientInput = z
  .object({
    tebraId: text(64).regex(/^[A-Za-z0-9_-]+$/),
...
export const activityInput = z
  .object({
    patientId: uuid,
    service: z.enum(["Injection", "UDS", "TMS", "Samples", "Forms"]),
  })
  .strict();
...
export interface Activity extends ActivityInput {
  ...
  handoff: "pending" | "prepared" | "filed";
  tebraReference: string | null;
  version: number;
  createdAt: string;
  updatedAt: string;
}
...
export const productInput = z
  .object({
    name: text(160),
    strength: text(80),
    unit: z.enum(["syringe", "kit", "tablet", "capsule", "vial"]),
    ndc: text(30).nullable(),
  })
  .strict();
...
export const lotInput = z
  .object({
    productId: uuid,
    lotNumber: text(80),
    expiresOn: date,
    location: text(100),
    ownership: z.enum(["clinic", "sample", "patient"]),
    ownerPatientId: uuid.nullable(),
  })
  .strict()
  .refine((x) => (x.ownership === "patient") === (x.ownerPatientId !== null), {
    message:
      "Patient-owned stock requires its patient; other stock cannot have an owner.",
  });
...
export const movementInput = z
  ...
  .strict()
  .superRefine((x, ctx) => {
    if ((x.kind === "reverse") !== (x.reversesId !== null))
      ctx.addIssue({
        code: "custom",
        message: "A reversal must identify the original movement.",
      });
    if (x.kind === "reverse" && x.quantity !== 0)
      ctx.addIssue({
        code: "custom",
        message:
          "Reversal quantities are calculated from the original movement.",
      });
    if (
      x.kind !== "reverse" &&
      (x.quantity === 0 || (x.kind !== "adjust" && x.quantity < 0))
    )
      ctx.addIssue({
        code: "custom",
        message: "Enter a positive whole number of stock units.",
      });
    if (["reserve", "release", "use"].includes(x.kind) && !x.patientId)
      ctx.addIssue({ code: "custom", message: "Select the patient." });
  });
