...
const allIm = (): AdministrationRule => ({
  routes: ["IM"],
  sites: [...IM_SITES],
});
const gluteal = (): AdministrationRule => ({
  routes: ["IM"],
  sites: [...GLUTEAL_SITES],
});
...
const buildMedication = (
  ...
  return {
    ...
    doses: [...catalog.doses],
    ...(catalog.dosesByInterval
      ? { dosesByInterval: catalog.dosesByInterval }
      : {}),
    windowBefore: catalog.windowBefore,
    windowAfter: catalog.windowAfter,
    ...(catalog.windowsByInterval
      ? { windowsByInterval: catalog.windowsByInterval }
      : {}),
    ...
    ...(catalog.cadenceByInterval
      ? { cadenceByInterval: catalog.cadenceByInterval }
      : {}),
    administrationRule: (dose) =>
      ruleForSiteGuidance(
        catalog.siteGuidance,
        dose,
        catalog.allowedRoutes ?? [catalog.route],
      ),
...
const catalogedMedications = Object.fromEntries(
  Object.entries(INJECTION_CLINICAL_REFERENCE_BUNDLE.medications).map(
    ([, reference]) => [reference.key, buildMedication(reference)],
  ),
) as Record<Exclude<InjectionMedicationKey, "other">, InjectionMedication>;

export const INJECTION_MEDICATIONS: Record<
...
export const allowedDosesForInterval = (
  ...
  medication.dosesByInterval
    ? (medication.dosesByInterval[intervalKey] ?? [])
    : medication.doses;
...
export const compatibleIntervalsForDose = (
  ...
  return INJECTION_INTERVAL_OPTIONS.map((option) => option.key).filter(
    (intervalKey) =>
...
export const preferredIntervalForDose = (
  ...
  if (compatibleIntervals.includes(medication.intervalKey))
    return medication.intervalKey;
...
export const injectionMuscleKey = (site: string): string => {
  ...
  const side = normalized.startsWith("r ")
    ? "right"
...
export const injectionSiteGroup = (site: string): InjectionSiteGroup | "" => {
  ...
  if (normalized.includes("subq") || normalized.includes("subcutaneous"))
    return "subq";
  return "";
};
