import type { InjectionEncounter } from "./injection.js";
import {
...
export type InjectionPatientScreenSectionKind =
  "screening" | "plan" | "additional" | "consent";
...
const normalizedCopy = (
  ...
  ...copy,
  staffInterpretation: normalizeStaffInterpretation(
    copy.staffInterpretation,
    language,
  ),
});
...
const antipsychoticSafetyRules = antipsychoticMedicationKeys.flatMap(
  (medicationKey) => [
    productLabelRule(
      medicationKey,
...
const aripiprazoleStartPlanRules = (
  ["aristada", "maintena", "asimtufii"] as const
).map((medicationKey) =>
  productLabelRule(
    medicationKey,
...
const paliperidoneStartPlanRules = (["sustenna", "erzofri"] as const).map(
  (medicationKey) =>
    productLabelRule(
      medicationKey,
      `phase-${medicationKey}-start-or-restart-plan`,
...
const longerIntervalStartPlanRules = (["trinza", "hafyera"] as const).map(
  (medicationKey) =>
    productLabelRule(
      medicationKey,
...
const initiationConsentRules: InjectionPatientScreenRule[] = [
  ...
  ...(["aristada", "initio", "maintena", "asimtufii"] as const).map(
    (medicationKey) =>
      productLabelRule(
        medicationKey,
  ...
  ...(["sustenna", "erzofri", "trinza", "hafyera"] as const).map(
    (medicationKey) =>
      productLabelRule(
        medicationKey,
...
const conditionalRules: InjectionPatientScreenRule[] = medicationKeys.flatMap(
  (medicationKey) => {
    const reference =
      INJECTION_CLINICAL_REFERENCE_BUNDLE.medications[medicationKey];
...
const labelsFor = (
  language: PatientScreenLanguage,
): InjectionPatientScreenLabels =>
  language === "es"
    ? {
        title: "EVALUACI\u00d3N PREVIA A LA INYECCI\u00d3N Y CONSENTIMIENTO",
        subtitle:
          "Formulario en papel de evaluaci\u00f3n y consentimiento del paciente",
        ...
        noProductSource:
          "No hay fuente de producto para un medicamento registrado como Otro.",
      }
    : {
        ...
        noProductSource:
          "No product source is available for a medication entered as Other.",
      };

const phaseLabels: Record<
  PatientScreenLanguage,
  Record<InjectionClinicalPhase, string>
> = {
  en: {
...
const blank = (value: string | undefined): string =>
  value?.trim() || "________________";
...
const matchesRule = (
  ...
  if (
    selector.medicationKeys?.length &&
    !selector.medicationKeys.includes(medicationKey as InjectionMedicationKey)
  ) {
    return false;
  }
  if (selector.doses?.length && !selector.doses.includes(encounter.dose.trim()))
    return false;
  if (
    selector.routes?.length &&
    !selector.routes.includes(encounter.route.trim())
  )
    return false;
  if (
    selector.intervalKeys?.length &&
    (!encounter.intervalKey ||
      !selector.intervalKeys.includes(encounter.intervalKey))
  ) {
    return false;
  }
...
export const canBuildInjectionPatientScreenDocument = (
  encounter: InjectionEncounter,
): boolean =>
  Boolean(
...
export const buildInjectionPatientScreenDocument = (
  ...
  const medicationKey = encounter.medicationKey;
  const showsConsent =
    medicationKey !== "other" && initiationLikePhases.includes(phase);
  const reference =
    medicationKey && medicationKey !== "other"
      ? INJECTION_CLINICAL_REFERENCE_BUNDLE.medications[medicationKey]
      : undefined;
  const medication =
    (reference?.catalog.label ?? encounter.customMedication?.trim()) || "Other";
  const sections = new Map<
    InjectionPatientScreenSectionKind,
    InjectionPatientScreenSection
  >();
  ...
  return {
    ...
    metadata: {
      ...
      interval: encounter.intervalKey
        ? injectionIntervalLabel(encounter.intervalKey)
        : "________________",
