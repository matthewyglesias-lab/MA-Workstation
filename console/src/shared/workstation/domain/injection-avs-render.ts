...
const estimatedBlockLines = (block: AvsBlock): number =>
  ...
  (block.paragraphs ?? []).reduce(
    (total, value) => total + estimatedTextLines(value),
    0,
  ) +
  (block.items ?? []).reduce(
    (total, value) => total + estimatedTextLines(value),
...
export type InjectionAvsLayoutVariant =
  "routine-one-page" | "routine-two-page" | "complex-two-page";
...
export const renderInjectionAvsHtml = (
  ...
  const recordNumber =
    model.identity.find((row) => row.label === "RECORD NO")?.value ?? "";
  const visitDate =
    model.identity.find((row) => row.label === "VISIT DATE")?.value ?? "";
