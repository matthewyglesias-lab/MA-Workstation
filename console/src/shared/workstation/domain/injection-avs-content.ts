...
export const siteRegion = (site: string): AvsSiteRegion => {
  ...
  if (value.includes("gluteal") || value.includes("buttock"))
    return "ventrogluteal";
  if (value.includes("deltoid")) return "deltoid";
  if (value.includes("abdomen") || value.includes("abdominal"))
    return "abdomen";
...
export const describeSite = (site: string, route: string): string => {
  ...
  const routeWord = /subq|subcut/i.test(String(route ?? ""))
    ? "subcutaneous"
    : "intramuscular";
  if (region === "abdomen") {
...
const siteCareForRegion = (
  region: AvsSiteRegion,
...
const profileFor = (key: string): MedProfile =>
  MED_PROFILES[key] ?? DEFAULT_PROFILE;
...
const initiationPlan = (input: InjectionAvsInput): InitiationPlan | null => {
  ...
  if (
    ...
    protocol === "maintena-14day" ||
    protocol === "asimtufii-14day"
  ) {
...
export const buildInjectionAvsModel = (
  ...
  const firmness: AvsDateFirmness = plan?.firm
    ? "firm"
    : coldChain
      ? "call-first"
      : "standard";
  ...
  const contactLines: AvsDataRow[] = coldChain
    ? [
        {
          label: "APPOINTMENTS & QUESTIONS",
          value: `${phone} - call before coming in`,
        },
        {
          label: "CLINIC HOURS",
          value: "Monday-Friday, 9:30 AM - 4:30 PM (appointments preferred)",
        },
        {
          label: "AFTER HOURS",
          value:
            "For concerns that cannot wait, use urgent care. Call 911 for emergencies.",
        },
      ]
    : [
  ...
  const when = [
    formatShortDate(input.administrationDate),
    input.administrationTime,
  ]
    .filter(Boolean)
    .join("  ");
  if (when) administration.push({ label: "DATE / TIME", value: when });
  const traceParts = [input.lot, formatExpiration(input.expiration)].filter(
    Boolean,
  );
  if (traceParts.length) {
    administration.push({
      label: "LOT / EXPIRATION",
      value: traceParts.join(" / "),
    });
  }
  if (hasSecondInjection(input)) {
    const secondTrace = [
      ...
      input.secondExpiration &&
        `exp ${formatExpiration(input.secondExpiration)}`,
    ]
      .filter(Boolean)
      .join(" - ");
    if (secondTrace)
      administration.push({ label: "SECOND INJECTION", value: secondTrace });
  }
  if (String(input.oralStatus ?? "").trim() === "administered") {
  ...
  const callItems = [
    ...(notGiven
      ? profileCallItems.filter(
          (item) => !/injection site|the site/i.test(item),
        )
      : profileCallItems),
  ...
  const documentSubtitle = notGiven
    ...
    : (plan?.subtitle ??
      REASON_SUBTITLE[String(input.reason ?? "").trim()] ??
      "");
  ...
  return {
    ...
    identity: [
      ...
      {
        label: "DOB",
        value: formatShortDate(input.patientDob) || input.patientDob || "-",
      },
      { label: "PROVIDER", value: input.orderingProvider || "-" },
      { label: "RECORD NO", value: input.recordNumber || "-" },
      {
        label: "VISIT DATE",
        value: formatShortDate(input.administrationDate) || "-",
      },
