import { injectionSiteGroup } from "./injection-catalog.js";
import type { InjectionMedication } from "./injection-catalog.js";
import type {
...
const impliedSiteGroup = (
  ...
  const groups = new Set(
    sites.map((entry) => injectionSiteGroup(entry)).filter(Boolean),
  );
...
export const resolveNeedle = (
  ...
  const siteGroup =
    injectionSiteGroup(site) || impliedSiteGroup(medication, dose);
...
export const techniqueNotesFor = (
  ...
  return administration.notes.filter((note) => {
    if (note.doses?.length && !note.doses.includes(normalizedDose))
      return false;
    ...
    if (
      note.siteGroups?.length &&
      siteGroup &&
      !note.siteGroups.includes(siteGroup)
    ) {
      return false;
    }
    return true;
...
export const formatNeedleSpec = (needle: NeedleSpec): string => {
  ...
  return measurement
    ? `${measurement} (${needle.descriptor})`
    : needle.descriptor;
};

const routeLabel = (
  siteGroup: InjectionSiteGroup | undefined,
  route: string,
): string => {
