...
const formatCategorizedAssessment = (
  ...
  const clinicianAttention = joinAssessmentFacts(
    review.clinicianAttention ?? [],
  );
  ...
  return joinBlocks([
    ...
    review.productPreparation
      ...
      : "",
    review.siteAssessment
      ? `Site assessment: ${cleanText(review.siteAssessment)}`
      : "",
...
const formatInitiation = (
  ...
  return block("Initiation protocol", [
    ...
    row("Scheduled / administered date", protocol.scheduledOrAdministeredDate),
...
const wasteDetailLine = (
  handling: InjectionDocumentationInput["handling"],
): string => {
...
const formatHandling = (input: InjectionDocumentationInput): string =>
  block("Product handling", [
    row("Product source", input.handling?.source),
    row("Medication waste", wasteDetailLine(input.handling)),
  ]);

const formatProductIssue = (input: InjectionDocumentationInput): string =>
...
const formatException = (input: InjectionDocumentationInput): string => {
...
const formatFollowUp = (input: InjectionDocumentationInput): string => {
...
const formatDisposition = (disposition?: InjectionDisposition): string => {
  ...
  return block("Disposition — medication not administered", [
    row(
      "Disposition",
      disposition.label || dispositionLabels[disposition.kind],
    ),
...
const formatIdentifiedProduct = (components: InjectionComponent[]): string => {
...
const formatNoteFactsPlan = (
  ...
  joinBlocks([
    ...
    facts.traceability
      ? `Traceability: ${endSentence(facts.traceability)}`
      : "",
    facts.followUp ? `Follow-up: ${facts.followUp}` : "",
    noteLines([
      facts.orderingProvider
        ? `Ordering provider: ${endSentence(facts.orderingProvider)}`
        : "",
      facts.administeredBy
        ? `Administered by: ${endSentence(facts.administeredBy)}`
        : "",
    ]).join("\n"),
  ]);
