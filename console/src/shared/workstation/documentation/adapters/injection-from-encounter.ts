...
} from "../../domain/injection-catalog.js";
import {
  injectionClinicalPhaseForReason,
  medicationVerificationsForPhase,
} from "../../domain/injection-clinical-reference.js";
import { resolveProviderDisplay } from "../../domain/provider-register.js";
import type { ClinicalEvaluation } from "../../domain/contracts.js";
import {
  mapLegacyInitiationProtocol,
  type LegacyInitiationSnapshot,
} from "./initiation-protocol.js";
import type {
  InjectionComponent,
...
const lateDoseReviewDocumentationText = (
  ...
  if (
    ...
    !hasCurrentLateDoseReview(encounter)
  ) {
    return "";
  }
...
const VERIFICATION_ASSESSMENT_FACTS: Partial<
  Record<MedicationVerificationKey, string>
> = {
  opioidFree:
  ...
  visualInspection:
    "Product visual inspection verified against the current label.",
  invegaInit:
...
const VERIFICATION_PLAN_FACTS: Partial<
  Record<MedicationVerificationKey, string>
> = {
  suppliedNeedle:
...
const ATTESTATION_NOTE_ORDER: ReadonlyArray<
  keyof InjectionEncounter["attestations"]
...
const compactAssessmentFacts = (
  ...
  ATTESTATION_NOTE_ORDER.forEach((key) => {
  ...
  const applicableVerifications = applicableVerificationKeys(
    encounter,
    medication,
  );
  (Object.keys(encounter.verifications) as MedicationVerificationKey[]).forEach(
    (key) => {
      if (!encounter.verifications[key] || !applicableVerifications.has(key))
        return;
      const fact =
        medicationVerificationDocumentation(
          medication,
...
const timingNoteText = (
  ...
  const reviewDocumentation = lateDoseReviewDocumentationText(
    evaluation,
    encounter,
  );
  const lateDoseReviewText = reviewDocumentation
    ? ` ${reviewDocumentation}`
    : "";
  ...
  const daysPhrase =
    days === null ? "" : `${days} day${days === 1 ? "" : "s"} since prior inj`;
  if (timing.state === "ok") {
  ...
  const reinit =
    phase === "reinitiation" ? "Re-initiation interval" : "Interval";
  return daysPhrase
...
const categorizedAssessmentFacts = (
  ...
  (
    Object.keys(encounter.attestations) as Array<
      keyof InjectionEncounter["attestations"]
    >
  ).forEach((key) => {
    if (!encounter.attestations[key]) return;
...
const primaryMedicationComponent = (
  ...
  const device =
    details.device === "Other"
      ? trimmed(details.deviceOther)
      : trimmed(details.device);
  const siteCondition =
  ...
  const volumeUnitLabel = details.volumeUnit
    ...
    : "";
  const amount = [trimmed(details.volume), volumeUnitLabel]
    .filter(Boolean)
    .join(" ");
  ...
  return {
    ...
    administrationTime:
      administered && encounter.administrationTime
        ...
        : undefined,
    administeredBy: administered
      ? trimmed(encounter.administeredBy) || undefined
      : undefined,
...
const PHASE_PRESENTATION_TEXT: Record<
  InjectionEvaluationOutput["phase"],
  string
> = {
...
const headlineAndPresentation = (
  ...
  const isCatalogProduct =
    encounter.medicationKey !== "other" && encounter.medicationKey !== "";
  ...
  const productLine = [
    medicationLabel,
    trimmed(encounter.dose),
    trimmed(encounter.route),
  ]
    .filter(Boolean)
    .join(" ");
  const site = trimmed(encounter.site);
...
const followUpLine = (encounter: InjectionEncounter): string => {
  ...
  const overrideText =
    ...
    trimmed(override.overrideReason)
      ...
      : "";
  if (!details.educationProvided)
    return [nextDueText, overrideText].filter(Boolean).join(" ");
  const eduText =
...
const responseNoteText = (encounter: InjectionEncounter): string => {
  if (encounter.response.kind === "custom")
    return trimmed(encounter.response.custom);
...
const techniqueNoteText = (
  ...
  const lines = [
    technique ? `Needle / technique: ${endSentence(technique)}` : "",
  ];
  const applicableVerifications = applicableVerificationKeys(
    encounter,
    medication,
...
const departureStatusLine = (encounter: InjectionEncounter): string => {
  ...
  if (details.departureStatus === "custom")
    return trimmed(details.departureStatusNote);
  return (
    INJECTION_DEPARTURE_STATUS_OPTIONS.find(
...
const clinicianAttentionText = (encounter: InjectionEncounter): string => {
  ...
  return unique(
    INJECTION_SAFETY_TRIGGERS.filter((trigger) =>
      activeSafetyConcerns.has(trigger.key),
...
const buildInjectionNoteFacts = (
  ...
  const { verification, clinicalReview, productPreparation, vitals } =
    compactAssessmentFacts(encounter, medication);
  const { headline, presentation } = headlineAndPresentation(
    encounter,
    evaluation,
    medicationLabel,
  );
  ...
  const secondAdministration =
    secondComponent && secondAdministrationLine && trimmed(secondComponent.site)
      ...
      : "";
  const administration = [primaryAdministration, secondAdministration]
    .filter(Boolean)
    .join(" ");
  ...
  return {
    ...
    timing:
      encounter.priorDoseDate || evaluation.output.lateDoseWarning
    ...
    orderingProvider:
      resolveProviderDisplay(encounter.orderingProvider) || undefined,
...
export function injectionEncounterToDocumentationInput(
  ...
  if (administered && !hasCurrentInjectionAdministrationReview(encounter))
    return null;
  ...
  const disposition = !dispositionKind
    ...
    : administered
      ...
      : {
          kind: (dispositionKind === "provider"
            ? "provider-directed"
            : dispositionKind) as "held" | "escalated" | "provider-directed",
          label:
  ...
  const primary = primaryMedicationComponent(
    encounter,
    evaluation,
    administered,
  );
  ...
  const initiationSnapshot =
    evaluation.output.phase === "initiation" ||
    evaluation.output.phase === "reinitiation"
  ...
  const lateDoseReviewText = lateDoseReviewDocumentationText(
    evaluation,
    encounter,
  );
  ...
  return {
    ...
    preAdministration: {
      ...
      productPreparation:
        assessmentFacts.productPreparation.join(" ") || undefined,
      siteAssessment: details.siteAssessed
      ...
      allergyReviewed: Boolean(
        dispositionKind && encounter.attestations.allergy,
      ),
      allergiesReview: dispositionKind
        ? trimmed(encounter.allergies) || undefined
        : undefined,
      previousDoseDate: encounter.priorDoseDate
        ? formatIsoDate(encounter.priorDoseDate)
        : undefined,
      previousSite: trimmed(encounter.priorSite) || undefined,
      timingReview: encounter.priorDoseDate
        ? [evaluation.output.timing.message, lateDoseReviewText]
            .filter(Boolean)
            .join(" ") || undefined
        : undefined,
      vitals: {
    ...
    handling: {
      ...
      productIssue: hasIssue
        ? trimmed(details.productIssueDetail) || undefined
        : undefined,
      productIssueAction: hasIssue
        ? trimmed(details.productIssueAction) || undefined
        : undefined,
      productIssueNotified: hasIssue
        ? trimmed(details.productIssueRecipient) || undefined
        : undefined,
      ...
      productIssueDirection: hasIssue
        ? trimmed(details.productIssueDirection) || undefined
        : undefined,
      productIssueNextStep: hasIssue
        ? trimmed(details.productIssueNextStep) || undefined
        : undefined,
    },
    ...
    noteFacts: administered
      ? buildInjectionNoteFacts(
          encounter,
          evaluation,
          primaryMedicationName,
          initiation.secondComponent,
        )
      : undefined,
  };
}
