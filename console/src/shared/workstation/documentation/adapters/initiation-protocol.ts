/** Pure initiation mapping extracted without behavioral changes from the original MA-Workstation legacy documentation adapter. */
import type {
  InjectionComponent,
...
export const mapLegacyInitiationProtocol = (
  ...
  const oral = ["maintena-14day", "asimtufii-14day", "aristada-21day"].includes(
    protocolKey,
  );
  ...
  if (orderCategory)
    notes.push(`Ordered initiation category: ${orderCategory}.`);
  ...
  const oralPlan = oral
    ? `${
        protocolKey === "aristada-21day" ? "21-day" : "14-day"
      } oral continuation ${
        oralStatus === "administered"
          ? "documented as administered today"
          : oralStatus === "verified"
