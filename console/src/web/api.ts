...
export async function initialize() {
  ...
  if (config.mode === 'sql') {
    msal = new PublicClientApplication({ auth: { clientId: config.webClientId!, authority: `https://login.microsoftonline.com/${config.tenantId}`, redirectUri: location.origin }, cache: { cacheLocation: BrowserCacheLocation.SessionStorage } });
...
export async function signIn() { await msal?.loginRedirect({ scopes: [config.apiScope!] }); }
async function authorization(): Promise<Record<string,string>> {
  if (!msal) return {};
  const account = msal.getActiveAccount() || msal.getAllAccounts()[0];
  if (!account) throw new Error('Sign in with your clinic Microsoft account to continue.');
  try {
    const result = await msal.acquireTokenSilent({ account, scopes: [config.apiScope!] });
    return { Authorization: `Bearer ${result.accessToken}` };
  } catch (error) {
    if (error instanceof InteractionRequiredAuthError) throw new Error('Your session needs attention. Sign in again to continue.');
    throw error;
  }
}
export async function request<T>(path: string, method = 'GET', body?: unknown, key?: string): Promise<T> {
  ...
  if (!response.ok) throw new Error(result.message || 'The request could not be confirmed.');
  return result as T;
}
