...
const repo = config.mode === 'demo' ? await seededDemo(config.timezone) : new SqlRepository(await connectSql(), config.clinicId, config.timezone);
...
console.log(`Clinic console listening on ${config.host}:${config.port} (${config.mode}).`);
