...
describe("API trust boundaries", () => {
  ...
  it("uses the clinic date across UTC midnight", () => {
    expect(
      clinicDate("America/Los_Angeles", new Date("2026-09-15T01:00:00Z")),
    ).toBe("2026-09-14");
  });
  it("requires audience validation separately, correct tenant, application, scope and staff roles", () => {
    ...
    for (const change of [
      ...
      expect(() => validateClaims({ ...claims, ...change }, c)).toThrow();
  });
    ...
    const app = await buildApp(
      {
        ...
        webClientId: randomUUID(),
      },
      new DemoRepository(),
    );
    const result = await app.inject({ url: "/api/v1/overview" });
    expect(result.statusCode).toBe(401);
    await app.close();
  });
  it("blocks reader writes and untrusted origins", async () => {
    const app = await buildApp(config, new DemoRepository(), async () => ({
      id: "reader",
      roles: ["Console.Reader"],
    }));
    expect(
  ...
  it("enforces schema, retry keys and uniqueness at the API boundary", async () => {
    ...
    expect(
      (await app.inject({ method: "POST", url: "/api/v1/patients", payload }))
        .statusCode,
    ).toBe(400);
    ...
    expect(
      (
        await app.inject({
          ...
          url: "/api/v1/patients",
          headers,
          payload: { ...payload, displayName: "Changed" },
        })
      ).statusCode,
    ).toBe(409);
    expect(
...
describe("Tebra handoff remains explicit", () => {
  ...
  it("does not require filing just to complete a service", () => {
    expect(() => validateActivityUpdate(activity, update)).not.toThrow();
  });
  it("requires a filing reference and completed service", () => {
    expect(() =>
      validateActivityUpdate(activity, { ...update, handoff: "filed" }),
    ).toThrow("filing reference");
  });
  it("rejects stale writes and overwriting filed handoffs", () => {
    expect(() =>
      validateActivityUpdate(activity, { ...update, expectedVersion: 1 }),
    ).toThrow("Another staff");
    expect(() =>
      validateActivityUpdate({ ...activity, handoff: "filed" }, update),
    ).toThrow("overwritten");
  });
});
