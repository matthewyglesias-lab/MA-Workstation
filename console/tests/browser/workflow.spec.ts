...
test('patient, inventory and explicit Tebra handoff work together',async({page})=>{
  ...
  await page.getByLabel('Patient display name').fill('Synthetic Browser Patient');
  ...
  await page.getByRole('button',{name:'Synthetic Browser Patient',exact:true}).click();
  ...
  await page.getByRole('button',{name:'Save',exact:true}).click();
  await expect(page.getByText('Prepared · not filed',{exact:true})).toBeVisible();
  await page.getByRole('button',{name:'Update activity',exact:true}).click();
  await page.getByLabel('Tebra documentation handoff').selectOption('filed');
  await page.getByLabel('Tebra filing reference').fill('Synthetic encounter reference');
  await page.getByRole('button',{name:'Save',exact:true}).click();
  await expect(page.getByText('Filed in Tebra',{exact:true})).toBeVisible();
  await page.getByRole('button',{name:'Inventory',exact:true}).click();
  await page.getByRole('button',{name:'Record movement',exact:true}).first().click();
  await page.getByLabel('Movement',{exact:true}).selectOption('reserve');
  await page.getByLabel('Quantity in whole stock units').fill('2');
  await page.getByLabel('Patient',{exact:true}).selectOption({label:'Synthetic Browser Patient · #BROWSER-TEST'});
  await page.getByLabel('Reason / operational reference').fill('Synthetic patient reservation');
  await page.getByRole('button',{name:'Save movement'}).click();
  await expect(page.getByRole('dialog')).not.toBeVisible();
  await expect(page.getByText('Synthetic patient reservation',{exact:true})).toBeVisible();
  await page.getByRole('button',{name:'Record movement',exact:true}).first().click();
  await page.getByLabel('Movement',{exact:true}).selectOption('use');
  await page.getByLabel('Quantity in whole stock units').fill('1');
  await page.getByLabel('Patient',{exact:true}).selectOption({label:'Synthetic Browser Patient · #BROWSER-TEST'});
  await page.getByLabel('Reason / operational reference').fill('Synthetic recorded use');
  await page.getByRole('button',{name:'Save movement'}).click();
  await expect(page.getByRole('dialog')).not.toBeVisible();
  await expect(page.getByText('Synthetic recorded use',{exact:true})).toBeVisible();
  await page.screenshot({path:'test-results/console-inventory.png',fullPage:true});
