...
Verified 15 September 2026. The approved **$7/month Render service is live with the injection workflow upgrade, Azure SQL, and 4–12 digit PIN access**. The deployed revision passed both console CI jobs, and hosted health, public configuration, unauthenticated-access, and sign-in-screen checks passed. Authenticated live use of the new workflow and physical note/AVS print review remain open.

This is the synthetic evaluation deployment for [PR #67](https://github.com/matthewyglesias-lab/MA-Workstation/pull/67), branch `feat/azure-sql-clinic-console`. The current live deployment is `dep-dakq6v95efls73d87jsg`, running commit `dfeab33205b4958d6b67412e30c2ef6f3db5115a` (tree `f186fc386997c80dbce07d945ed422da69146c32`); it became live at **20:00:10 UTC** after a **19:58:53 UTC** trigger. Earlier staff verification and hosted workflow/restart evidence are attributed to their original revisions below. The existing workstation and letter-builder deployments were left unchanged.
...
Keep one $7 service and the existing free-offer database with paid overages off. The source remains on draft PR #67; automatic deployment and preview replicas are off. The unrelated legacy Static Web App preview-slot failure remains separate from this console deployment. The current injection upgrade is deployed with passing exact-commit CI and public endpoint/sign-in checks; earlier named-account, synthetic workflow, restart and cleanup evidence retains its original scope. Authenticated live workflow/print, second-workstation, sustained-load and backup/restore checks are still open.
