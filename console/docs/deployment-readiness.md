...
Status checked 15 September 2026: **live synthetic evaluation with four-digit PIN support and three individually verified named staff accounts**. Staff enrollment, hosted sign-in/role/logout checks and temporary-access cleanup are complete; authenticated browser/print workflow, second-workstation, sustained-load and backup/restore checks remain open. See the [current Render deployment record](render-evaluation-status.md) for the live release, completed synthetic workflow/restart checks and final access settings.
...
This retained App Service/private-network bootstrap is an alternative design and has not been exercised. Its resource creation, private DNS resolution, SQL grants and staff setup would require separate execution and verification. The actual Render evaluation's completed SQL bootstrap, hosted checks and outstanding backup/restore exercise are recorded in the [current deployment status](render-evaluation-status.md).
