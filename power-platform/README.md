...
- a Draft-status row that still carries a residual Final JSON payload or Idempotency Key from a prior finalization fails closed with `stale-final-artifact` instead of being finalized over — reopening a finalized action requires a separate, explicit, audited reset/new-action process;
